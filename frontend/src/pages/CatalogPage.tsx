...
function EditablePrice({ item }: { item: CatalogAccessory }) {
  ...
  const handleSave = () => {
    ...
    updatePrice.mutate(
      ...
      {
        onSuccess: (data) => {
          toast(`Prix mis à jour (${data.ads_refreshed} annonces recalculées)`, 'success')
          setEditing(false)
        },
  ...
  const handleReset = () => {
    resetPrice.mutate(item.group, {
      onSuccess: (data) => {
        toast(`Prix réinitialisé (${data.ads_refreshed} annonces recalculées)`, 'success')
      },
      onError: (err) => toast((err as Error).message, 'error'),
    })
  }
    ...
    <div className="flex items-center gap-2">
      ...
      {item.has_override && (
        <button
          ...
          title={`Réinitialiser au prix par défaut (${formatPrice(item.default_new_price)})`}
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-text-dim">{items.length} accessoire{items.length > 1 ? 's' : ''}</span>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-6 shrink-0">
              ...
              <span className="text-xs text-text-dim w-20 text-right">
                occasion: {formatPrice(item.estimated_used_price)}
              </span>
            </div>
          </div>
        ))}
    ...
    refreshAll.mutate(undefined, {
      onSuccess: (data) => {
        const msg = data.ads_skipped_manual > 0
          ? `${data.ads_refreshed} annonces recalculées (${data.ads_skipped_manual} ignorées car éditées manuellement)`
          : `${data.ads_refreshed} annonces recalculées`
        toast(msg, 'success')
      },
      onError: (err) => toast((err as Error).message, 'error'),
    })
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-text-primary" style={{ fontFamily: 'Fraunces, Georgia, serif' }}>
            Référentiel accessoires
          </h1>
          <p className="text-sm text-text-muted mt-1">
            Cliquez sur un prix pour le modifier. Les changements sont propagés à toutes les annonces.
          </p>
        </div>
        <Button
          variant="secondary"
