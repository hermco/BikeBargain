  ...
  return (
    <Dialog.Root open={open} onOpenChange={(v) => { if (!v) handleClose(); else setOpen(true) }}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] ${previewData ? 'max-w-4xl' : 'max-w-2xl'} max-h-[85vh] flex flex-col bg-surface border border-tint/[0.08] rounded-2xl z-50 shadow-2xl shadow-black/50`}>
              ...
              <form onSubmit={handleExtract} className="space-y-5">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-tint/[0.04] border border-tint/[0.08] pl-10 pr-4 py-3 text-sm text-text-primary placeholder-text-dim focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500/30 transition-all"
                    required
                    autoFocus
                  />
                </div>
          ...
          {previewData && (
            ...
            <div className="flex-1 overflow-y-auto px-7 space-y-5">
              ...
              {/* Image gallery */}
              {images.length > 0 && (
                <div className="rounded-xl bg-tint/[0.03] border border-tint/[0.06] overflow-hidden">
                  <div className="relative">
                    <img
              ...
              {/* Main info */}
              <div className="rounded-xl bg-tint/[0.03] border border-tint/[0.06] p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-y-2.5 gap-x-4 text-sm">
                  ...
                  <span className="font-semibold text-accent-text">{formatPrice(previewData.price as number)}</span>
                    ...
                    <div className="flex gap-1.5">
                      {wheelTypes.map((w) => (
                        <button key={w} onClick={() => updateField('wheel_type', w)}
                          className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${w === previewData.wheel_type ? 'bg-amber-500/20 text-accent-text ring-1 ring-amber-500/40' : 'bg-tint/[0.04] text-text-muted hover:bg-tint/[0.08]'}`}>
                          {w}
                        </button>
                      ))}
                    </div>
              ...
              {/* Accessories */}
              <div className="rounded-xl bg-tint/[0.03] border border-tint/[0.06] p-4">
            ...
            <div className="sticky bottom-0 shrink-0 px-7 py-4 bg-surface border-t border-tint/[0.06] rounded-b-2xl">
